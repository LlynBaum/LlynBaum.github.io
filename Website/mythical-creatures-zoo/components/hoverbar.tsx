import { StaticImageData } from "next/image";

interface HoverBarProp{
    category: string
    img: StaticImageData
    bgPos: React.CSSProperties
}

const HoverBar = ({HoverBarProp, styles} : {HoverBarProp: HoverBarProp, styles: string}) => {

    const {category, img, bgPos} = HoverBarProp;

    return(
        <div className={"flex justify-center items-center overflow-hidden relative group " + styles}>
            <img src={img.src} style={bgPos} className="object-cover group-hover:brightness-50 group-hover:scale-125 transition ease-in-out duration-500"></img>
            <h2 className="absolute font-Sedgwick text-slate-50 text-6xl opacity-0 group-hover:opacity-100 group-hover:scale-125 transition ease-in-out duration-500">{category}</h2>
        </div>
    )
}

export default HoverBar;